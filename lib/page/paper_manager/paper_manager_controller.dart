...
class PaperManagerController extends GetxController {
  ...
  Future<void> showDialogSelectNumber(
      BuildContext context, int index, int indexChild) async {
    List<int> list = [];
    list.add(0);
  ...
  Future<void> onUpdatePaper() async {
    try {
      CollectionReference userCollection =
          firestore.collection(DataRowName.Papers.name);

      SelectPaperChange changeData =
          SelectPaperChange.convertData(selectPaper.value);
      changeData.papers = listRow;

      await userCollection
          .doc(selectPaper.value.paperID)
          .update(changeData.toJson());
  ...
  Future<void> onSetPaper() async {
    try {
      ...
      CollectionReference categoryCollection =
          firestore.collection(DataRowName.CategoryPaper.name);
      ...
      await userCollection
          .doc(selectPaper.value.paperID)
          .set(changeData.toJson());
      await categoryCollection.doc(categoryID).set({"isAdd": true});
  ...
  Future<void> getCategoryPaper() async {
    ...
    for (var item in data.docChanges) {
      CategoryPaper categoryPaper =
          convertData(item.doc.data() as Map<String, dynamic>);
